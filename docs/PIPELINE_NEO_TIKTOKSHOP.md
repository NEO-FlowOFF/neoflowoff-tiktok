...
Este documento define o fluxo real do modulo `apps/content-engine` sem ambiguidade de execucao.
...
- Miner: `apps/content-engine/scripts/mine_tiktok_shop.py`
- Runner: `apps/content-engine/scripts/run_neo_tiktokshop.py`
- Dependencias Python: `apps/content-engine/requirements.txt`
- Runtime:
  - Entrada CSV: `apps/content-engine/runtime/inputs/pending_products.csv`
  - Audio: `apps/content-engine/runtime/assets/*_audio.mp3`
  - Video: `apps/content-engine/runtime/outputs/*_final_render.mp4`
  - Metadata: `apps/content-engine/runtime/outputs/*_meta.json`
  - CSV processado: `apps/content-engine/runtime/inputs/archive/processed_<timestamp>.csv`
...
1. Preencha `apps/content-engine/runtime/inputs/pending_products.csv`.
2. Rode:
...
apps/content-engine/.venv/bin/python apps/content-engine/scripts/run_neo_tiktokshop.py --skip-upload
```
...
## 10. Safe commit local

Padrao seguro sem push:

```bash
make safe-content-push MSG="content-engine: ajuste de fluxo"
```

Push remoto so com confirmacao explicita:

```bash
PUSH=1 make safe-content-push MSG="content-engine: ajuste de fluxo"
```

Validacao sem commit:

```bash
DRY_RUN=1 make safe-content-push MSG="teste"
```

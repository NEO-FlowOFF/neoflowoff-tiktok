...
- CSV atual: `apps/content-engine/runtime/inputs/pending_products.csv`
- Videos: `apps/content-engine/runtime/outputs/*_final_render.mp4`
- Metadados: `apps/content-engine/runtime/outputs/*_meta.json`
...
## 8. Git seguro (sem push por padrao)

Commit local com validacao:

```bash
make safe-content-push MSG="content-engine: ajuste"
```
...
Push remoto apenas com acao explicita:

```bash
PUSH=1 make safe-content-push MSG="content-engine: ajuste"
```
...
3. Confirme MP4 em `runtime/outputs`.
